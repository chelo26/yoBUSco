...
public class FilterActivity extends FragmentActivity implements android.view.View.OnClickListener,GoogleApiClient.OnConnectionFailedListener, GoogleApiClient.ConnectionCallbacks{
	...
	class GetMarkersBySearch extends AsyncTask<Void, Void, Integer>
	{
		...
		protected Integer doInBackground(Void... paramss) {
            ...
            if( cb_filterByType.isChecked()) {
                params.add(new AbstractMap.SimpleEntry("fire_event_checked", filterTypeDialog.isFireChecked() ? "1" : "0"));
                params.add(new AbstractMap.SimpleEntry("police_event_checked", filterTypeDialog.isPoliceChecked() ? "1" : "0"));
                params.add(new AbstractMap.SimpleEntry("emergency_event_checked", filterTypeDialog.isEmergencyChecked() ? "1" : "0"));
            }


            params.add(new AbstractMap.SimpleEntry("radius_checked", filterByRadius ? "1" :"0"));

            params.add(new AbstractMap.SimpleEntry("description_checked", cb_filterByDescription.isChecked()? "1" : "0"));
            params.add(new AbstractMap.SimpleEntry("description", filterDescriptionDialog.getDescription()));

            params.add(new AbstractMap.SimpleEntry("date_checked", cb_filterByDate.isChecked()? "1": "0"));
            if( cb_filterByDate.isChecked()) {
                DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
                if (filterDateDialog.isFilteringByFromDate()) {
                    params.add(new AbstractMap.SimpleEntry("from_date_checked", "1"));
                    params.add(new AbstractMap.SimpleEntry("begin_time", String.format("%s 00:00:00", dateFormat.format(filterDateDialog.getFromDate()))));
                }
                if (filterDateDialog.isFilteringByToDate()) {
                    params.add(new AbstractMap.SimpleEntry("to_date_checked", "1"));
                    params.add(new AbstractMap.SimpleEntry("end_time", String.format("%s 23:59:59", dateFormat.format(filterDateDialog.getToDate()))));
                }
            }
